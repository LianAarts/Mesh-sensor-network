...
void setupSensor680() {
  ...
  BME680.setOversampling(TemperatureSensor,
                         Oversample16);  // Use enumerated type values
  BME680.setOversampling(HumiditySensor,
                         Oversample16);  // Use enumerated type values
  BME680.setOversampling(PressureSensor,
                         Oversample16);  // Use enumerated type values
  debugln3("- Setting IIR filter to a value of 4 samples");
  BME680.setIIRFilter(IIR4);  // Use enumerated type values
  debugln3(
      "- Setting gas measurement to 320\xC2\xB0\x43 for 150ms");  // "�C"
                                                                  // symbols
 ...
 * v:   Value                     (21.8)
 *
 * @param isRoot If we are root we also send the IP address of the root so Home
 * Assistant can send API requests to the root.
 * @param nodeName This is part of the entity ID we want to use in Home
 * Assistant
...
String makeSensorMessage680(bool isRoot, String nodeName, String IP,
  ...
  String meas2 =
      "}, {\"n\": \"gas\", \"u\": \"ohm\", \"v\":" + String(gasBME680);
  String meas3 =
      "}, {\"n\": \"humidity\", \"u\": \"%\", \"v\":" + String(humBME680);
  String meas4 =
      "}, {\"n\": \"pressure\", \"u\": \"Hpa\", \"v\":" + String(presBME680);
  String meas5 =
      "}, {\"n\": \"chipID\", \"u\": \"ID\", \"v\":" + String(nodeID);
